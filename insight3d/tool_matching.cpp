...
{
	printf("Matching tab creation; File:tool_matchin.cpp\n");
...
void tool_matching_standard() 
{
	...
	printf(" extract keypoints from all images; Funtion:matching_extract_features File:tool_matching.cpp\n"); 
	...
	for ALL(uf_nodes, i)
	{
		...
		printf(" creating vertex Funtion:tool_matching_standard() File:tool_matching.cpp/n");
		size_t vertex_id; 
...
void matching_extract_features(const double max_size)
{
	...
	printf("starting progress tool bar; Funtion:matchin_extract_features\n");
	...
	for ALL(shots, i)
	{
		...
		printf("extract SIFT keypoints; call to sift_features() in File:insight3d/sift/src/sift.h; Funtion:matching_extract_features File:tool_matching.cpp\n");
		shot->keypoints_count = sift_features(img, &shot->keypoints);
		printf("[count = %d]\n", shot->keypoints_count);
		fflush(stdout);

		printf("build kd tree; call to kdtree_build in File:insight3d/sift/src/kdtree.c\n");
